...
focus_opacity_table = {{"firefox", 0.9}, {"feh", 1}}
unfocus_opacity_table = {{"firefox", 0.9}, {"feh", 1}}
...
M.adjust = function(c, delta)
    c.opacity = math.max(math.min(c.opacity + delta, 1), 0)
    table.insert(locked_opacity, c)
end

M.get_user_defined_opacity = function(c, is_focus)
	if c.class then
		local apps_opacity
		if is_focus then
			apps_opacity = focus_opacity_table
		else
			apps_opacity = unfocus_opacity_table
		end
		for _, app_opacity in ipairs(apps_opacity) do
			if c.class:lower():find(app_opacity[1]) then
				return app_opacity[2]
			end
		end
	end
	return nil
...
M.get_opacity_for_client = function(c)
	...
	for _, app_opacity in ipairs(focus_opacity_table) do
		if c.class:lower():find(app_opacity[1]) then
			opacity = app_opacity[2]
		end
	end
        --if c.class and c.class:lower():find("feh") then
        --    opacity = 1
        --elseif c.class and c.class:lower():find("firefox") then
	--    opacity = 0.9
        --else
        --    opacity = M.opacity.focused
        --end
	...
	opacity = M.opacity.focused
	for _, app_opacity in ipairs(unfocus_opacity_table) do
		if c.class:lower():find(app_opacity[1]) then
			opacity = app_opacity[2]
		end
	end
